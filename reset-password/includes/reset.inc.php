...
if (isset($_POST['resetsubmit'])) {
    ...
    */
    ...
    $sql = "SELECT * FROM zha_auth_tokens WHERE zha_auth_type='password_reset' AND zha_selector=? AND zha_expiry_at >= NOW() LIMIT 1";
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        if (!($row = mysqli_fetch_assoc($results))) {
            ...
            if ($tokenCheck === false) {
                ...
                $sql = 'SELECT * FROM zha_users WHERE zha_email=?;';
                $stmt = mysqli_stmt_init($conn);
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)){
                    ...
                    if (!$row = mysqli_fetch_assoc($results)) {
                        ...
                        $sql = 'UPDATE zha_users SET password=? WHERE zha_email=?;';
                        $stmt = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($stmt, $sql))
                        {
                            $_SESSION['ERRORS']['scripterror'] = 'SQL ERROR';
                            ...
                            $sql = "DELETE FROM zha_auth_tokens WHERE zha_email=? AND zha_auth_type='password_reset';";
                            $stmt = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($stmt, $sql)){
